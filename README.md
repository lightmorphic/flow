...
```
sudo apt install ./lightmorphic-flow_0.8.1_all.deb
```
...
```
sudo dnf install ./lightmorphic-flow-0.8.1-1.noarch.rpm
```
...
your computers by name, brings it back, disconnects one computer on its own,
opens the settings window, and exits the whole app.
